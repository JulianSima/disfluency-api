...
public class DisfluencyApiApplication {
	...
	CommandLineRunner initDatabase(PatientRepo patientRepo,
		...
		return args -> {
			...
			List<ExerciseAssignment> messiAsignacion = Arrays.asList(new ExerciseAssignment(inicioSuave), new ExerciseAssignment(fonacionContinuada), new ExerciseAssignment(velocidadComoda));
			messi.addExercisesAssignment(messiAsignacion);
			exerciseAssignmentList.addAll(messiAsignacion);

			List<ExerciseAssignment> depaulAsignacion = Arrays.asList(new ExerciseAssignment(toquesLigeros));
	...
	/*@Bean
	CommandLineRunner initDatabase(PatientRepo patientRepo, TherapistRepo therapistRepo, ExerciseRepo exerciseRepo, UserRepo userRepo, ExerciseAssignmentsRepo exerciseAssignmentsRepo) {
		return args -> {
